...
class MissionControl extends Component {
  ...
  onPlanetChange = (e, combination) => {
    const { selectedPlanetsArray } = this.state;

    if (selectedPlanetsArray && selectedPlanetsArray.length) {
      const newSelectedPlanets = selectedPlanetsArray.map(
      ...
      this.setState(
        {
          selectedPlanetsArray: newSelectedPlanets
        },
  ...
  calculateTravelTime = () => {
    const { planets, vehicles, selectedPlanetsArray, selectedVehicles } = this.state;
    let travelTime = 0;
    for (let i = 0; i < selectedPlanetsArray.length; i++) {
      if (selectedPlanetsArray[i] !== "" && selectedVehicles[i] !== "") {
        travelTime +=
          planets.filter(planet => planet.name === selectedPlanetsArray[i])[0]
            .distance /
    ...
    this.props.findFalcone(
      {
        ...
        planet_names: this.state.selectedPlanetsArray,
    ...
    return (
      <div style={{ height: "inherit", marginTop: "20px" }}>
        <Paper
          ...
          style={{
            background: "linear-gradient(45deg, Grey 30%, #F37335 90%)"
          }}
        >
          {planets.length && vehicles.length ? (
            <Grid container justify="center">
              ...
              {selectedPlanetsArray.map((selectedPlanet, index) => (
                <Grid
                  key={index}
                  item
                  xs={12}
                ...
                >
                  ...
                  <PlanetShipSelection
                    ...
                    selectedPlanets={selectedPlanetsArray}
